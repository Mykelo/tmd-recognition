...
def get_tnf_grouped_labels(path: str, classes: list[str] = ['ALS', 'Healthy controls', 'Stroke']) -> pd.DataFrame:
    rows = []
    for i, cls in enumerate(classes):
        data_dir = os.path.join(path, cls)
        for data in os.listdir(data_dir):
            ...
            group = subject
            label = i
            rows.append([name, file, subject, group, cls, label])

    return pd.DataFrame(rows, columns=['Name', 'File', 'SubjectID', 'GroupID', 'Class', 'Label'])


def get_tnf_labels(path: str, classes: list[str] = ['ALS', 'Healthy controls', 'Stroke'], data_name='Bbox_gt') -> pd.DataFrame:
    data_df = pd.DataFrame()
    id = 0
    for i, cls in enumerate(classes):
        data_dir = os.path.join(path, cls, data_name)
        for data in os.listdir(data_dir):
            local_data_df = pd.read_csv(os.path.join(
                data_dir, data), sep=', ', engine='python')
            name = data.replace('_gt', '')
            name = name.replace('.txt', '')

            local_data_df['Name'] = name
            local_data_df['File'] = local_data_df.apply(
                lambda row: name + '.avi_' + str(int(row['Frame'])) + '.jpg', axis=1)
            local_data_df['SubjectID'] = data.split('_')[0]
            local_data_df['GroupID'] = local_data_df.apply(
                lambda row: str(id) + '_' + str(int(row['Frame'])), axis=1)
            local_data_df['Class'] = cls
            local_data_df['Label'] = i
...
class TNFDataset(FacialLandmarksDataset):
    def __init__(self, root_dir: str, target_size: int = 256):
        super().__init__(root_dir, target_size)
        self.bbox_data = get_tnf_labels(root_dir)
        self.landmarks_data = get_tnf_labels(
            root_dir, data_name='Landmarks_gt')
        self.target_size = target_size
        self.data_name = 'TNF'
        self.image_names = self.landmarks_data['File'].to_list()

    def __len__(self):
        return self.bbox_data.shape[0]

    def __getitem__(self, i):
        bbox_row = self.bbox_data.iloc[i]
        landmarks_row = self.landmarks_data.iloc[i]
        image_path = os.path.join(
            self.root_dir, bbox_row['Class'], 'Frames', bbox_row['File'])
        image = cv2.imread(image_path)
        bbox = bbox_row[['x1', 'y1', 'x2', 'y2']].to_list()
        anno = landmarks_row.to_list()[1:68*2+1]
        ...
        scale = 0
        xmin -= width_fill/2 + width * scale/2
        ymin -= height_fill/2 + height * scale/2
        xmax += width_fill/2 + width * scale/2
        ymax += height_fill/2 + height * scale/2
        ...
        anno = np.concatenate(
            [anno_x.reshape(-1, 1), anno_y.reshape(-1, 1)], axis=1).flatten()
        return cv2.resize(image_crop, (self.target_size, self.target_size)), anno


class MENPODataset(FacialLandmarksDataset):
    def __init__(self, root_dir: str, target_size: int = 256, face_side: str = 'both'):
        super().__init__(root_dir, target_size)
        self.target_size = target_size
        self.data_name = 'MENPO'
        self.images_path = os.path.join(root_dir, 'processed_images')
        all_files = os.listdir(self.images_path)
        self.image_names = [x for x in all_files if '.pts' not in x]
        self.label_files = [x for x in all_files if '.pts' in x]
    ...
    def __getitem__(self, i):
        ...
        label_path = os.path.join(self.images_path, label_file)
        image = cv2.imread(image_path)
        ...
        with open(label_path, 'r') as ff:
            ...
            bbox = file_lines[-1].strip().split()[1:]
            ...
            if self.face_side != 'both':
                img_face_side = 'left' if anno[0] > anno[19*2] else 'right'
                if self.face_side != img_face_side:
                    image, anno = flip_image(image, anno)
            return image, anno
            # return image, anno


class FITYMIDataset(FacialLandmarksDataset):
    def __init__(self, root_dir: str, target_size: int = 256):
        super().__init__(root_dir, target_size)
        self.target_size = target_size
        self.data_name = 'FITYMI'
        self.images_path = os.path.join(root_dir, 'processed_images')
        all_files = os.listdir(self.images_path)
        self.image_names = [
            x for x in all_files if '.png' in x and 'seg' not in x]
        self.label_files = [x for x in all_files if 'ldmks' in x]
